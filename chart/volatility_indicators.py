...
def handle_volatility_indicators2(args, apds, kdf):
    ...
    ss = []
    if args.ATR:
        real = talib.ATR(kdf["high"], kdf["low"], kdf["close"], timeperiod=14)
        ss.append(('ATR', real))
    if args.NATR:
        real = talib.NATR(kdf["high"], kdf["low"], kdf["close"], timeperiod=14)
        ss.append(('NATR', real))
    if args.TRANGE:
        real = talib.TRANGE(kdf["high"], kdf["low"], kdf["close"])
        ss.append(('TRANGE', real))
    return ss
