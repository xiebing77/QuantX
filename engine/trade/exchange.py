...
class ExchangeTradeEngine(TradeEngine):
    ...
    def new_limit_bill(self, side, symbol, multiplier, price, qty, rmk='', oc=None):
        ...
        if type(ret) in [str, int] :
            ...
            if type(ret[0]) in [str, int]:
                ...
                order_ids = [order[self.trader.Order_Id_Key] for order in ret]
        else:
            order = ret
            order_ids = order[self.trader.Order_Id_Key]
    ...
    def get_bills(self, bill_status=None):
    ...
    def get_open_bills(self):
        return self.get_bills(common.BILL_STATUS_OPEN)

    def get_close_bills(self):
        return self.get_bills(common.BILL_STATUS_CLOSE)
    ...
    def get_orders_from_db(self, order_ids):
        query = {
    ...
    def _get_commission_from_trades(self, multiplier, trades):
        commission = {}
        if len(trades) == 0:
            return commission
        ...
        if hasattr(self.trader, 'Trade_Key_CommissionQty'):
            ...
            commission = calc_commission_by_trades(multiplier, self.trader, trades,
    ...
    def get_bill_commission(self, bill):
        ...
        trades = self._get_trades_from_db(order_ids)
        multiplier = bill[common.BILL_MULTIPLIER_KEY]
        commission = self._get_commission_from_trades(multiplier, trades)
        return commission


    def get_order_commission(self, multiplier, order):
    ...
    def get_bill_deal_info(self, bill):
        ...
        total_quote_qty = 0
        order_ids = get_orderids_by_bill(bill)
        orders = self.get_orders_from_db(order_ids)
        if len(orders) == 0:
    ...
    def _init_position(self):
        ...
        close_bills = self.get_close_bills()
        ...
        for bill in close_bills:
            ...
            if not orders:
                ...
                continue
            for order in orders:
                commission = self.get_order_commission(multiplier, order)
    ...
    def sync_bill(self, order, trades):
        ...
        log.info('sync_bill => {}'.format(self.position))
    ...
    def handle_open_bills(self):
        ...
        open_bills = self.get_open_bills()
        if not open_bills:
            return [], []
        ...
        for open_bill in open_bills:
            ...
            for order_id in order_ids:
                ...
                if not self.trader.check_status_is_close(order):
                    log.debug('order is open, order: {}'.format(order))
                    continue
            ...
            log.info('handle_open_bills => {}'.format(self.position))
