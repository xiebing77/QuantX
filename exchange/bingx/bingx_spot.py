...
from .bingx import BingX, api_key, secret_key
from .spot import Spot
from common import create_balance
from common import TIME_IN_FORCE_GTC


class BingXSpot(BingX):
    name = BingX.name + '_spot'
    ...
    def _get_assetPrecision(self, ex_symbol):
        ...
        return int(-math.log10(float(sy_info['stepSize']))), int(-math.log10(float(sy_info['tickSize'])))
    ...
    def time(self):
        return self.get_time_from_data_ts(self.__api.time()['timestamp'])

    def _exchange_info(self, ex_symbol: str = None, ex_symbols: list = None):
        return self.__api.exchange_info(ex_symbol)['data']
    ...
    def _ticker_price(self, exchange_symbol):
        #print(self.__api.ticker_price(exchange_symbol))
        #return float(self.__api.ticker_price(exchange_symbol)['price'])
        return float(self._trades(exchange_symbol, 1)[0][self.Trade_Key_Price])
